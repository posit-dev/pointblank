...
### Data Management
- **Load DataFrames** from CSV, Excel, Parquet, JSON, JSONL files
...
| **Excel** | `.xlsx`, `.xls` | `monthly_report.xlsx` | pandas (openpyxl) |
...
The consistent natural language interface works the same regardless of file format, so you can focus on validation logic rather than technical details. Polars provides faster processing for large datasets, while pandas offers broader format support including Excel files.
