...
Pointblank currently supports 55 countries with full locale data for realistic test data generation.
...
With weighting enabled you will see popular names like James, John, Mary, and Patricia appear
...
while producing a realistic long-tail distribution.

When `weighted=False` (the default), all entries across every tier are pooled together and sampled
uniformly, preserving backward compatibility. Here is a side-by-side comparison of the same schema
generated with and without weighting:

```{python}
uniform = pb.generate_dataset(schema, n=500, seed=42, country="US")
weighted = pb.generate_dataset(schema, n=500, seed=42, country="US", weighted=True)
```

```{python}
uniform["city"].value_counts().head(10)
```

```{python}
weighted["city"].value_counts().head(10)
```

The uniform sample spreads selections roughly evenly across all cities, while the weighted sample
concentrates selections on the most populous metropolitan areas.
