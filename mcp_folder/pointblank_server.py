...
from typing import AsyncIterator, List, Dict, Any, Optional, Union # Keep Union for now
...
def load_dataframe(input_path: str, df_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    if effective_df_id in app_ctx.loaded_dataframes:
        raise ValueError(f"DataFrame ID '{effective_df_id}' already exists. Choose a different ID or omit to generate a new one.")
...
def create_validator(
    ...
    thresholds_dict: Optional[Dict[str, Union[int, float]]] = None, # e.g. {"warning": 1, "error": 20, "critical": 0.10}
    ...
    if df_id not in app_ctx.loaded_dataframes:
        raise ValueError(f"DataFrame ID '{df_id}' not found. Please load it first using 'load_dataframe'.")
    ...
    if effective_validator_id in app_ctx.active_validators:
        raise ValueError(f"Validator ID '{effective_validator_id}' already exists. Choose a different ID or omit to generate a new one.")
    ...
    if actions_dict:
        try:
            ...
            pb_actions = pb.Actions(**actions_dict) # This assumes pb.Actions can be created this way
        except Exception as e:
...
def add_validation_step(
    ...
    if validator_id not in app_ctx.active_validators:
        raise ValueError(f"Validator ID '{validator_id}' not found. Please create it first using 'create_validator'.")
    ...
    if validation_type not in supported_validations:
        raise ValueError(f"Unsupported validation_type: '{validation_type}'. Supported types include: {list(supported_validations.keys())}")
    ...
    try:
        ...
        raise ValueError(f"Error calling validation method '{validation_type}' with params {current_params}. Original error: {e}. Check parameter names and types against Pointblank's API for the '{validation_type}' method of the 'Validate' class.")
    except Exception as e:
        raise RuntimeError(f"An unexpected error occurred while adding validation step '{validation_type}': {e}")

    return {"validator_id": validator_id, "status": f"Validation step '{validation_type}' added successfully."}
...
async def get_validation_step_output(
    ...
    if not output_format:
        ...
        await ctx.warning(f"No file extension provided. Defaulting to {output_format}. Saving to: {p_output_path}")

    try:
        # Ensure validator has been interrogated.
        if not getattr(validator, 'time_processed', None):
            await ctx.warning(f"Validator '{validator_id}' has not been interrogated. Interrogating now.")
        ...
        if output_format == ".csv":
            ...
            if isinstance(data_extract_df, (pd.DataFrame)):
                ...
                raise TypeError(f"Unsupported DataFrame type '{type(data_extract_df).__name__}' for CSV export.")

        # --- Logic for PNG output (requires a step_index) ---
        elif output_format == ".png":
            if not step_index:
                raise ValueError("A 'step_index' is required to generate a PNG report for a specific step.")

            step_report_obj = validator.get_step_report(i=step_index)

            if step_report_obj is None:
                num_steps = len(validator.n())
                raise ValueError(f"No report found for step index {step_index}. Validator has {num_steps} step(s).")

            if not hasattr(step_report_obj, 'save'):
                 raise TypeError("The visual report object for this step does not have a .save() method.")

            step_report_obj.save(str(p_output_path))
            message = f"Visual report for step {step_index} saved to {p_output_path.resolve()}"

        else:
            raise ValueError(f"Unsupported file format '{output_format}'. Please use '.csv' or '.png'.")

        await ctx.report_progress(100, 100, message)
        return {"status": "success", "message": message, "output_file": str(p_output_path.resolve())}

    except Exception as e:
        raise RuntimeError(f"Error getting output for validator '{validator_id}': {e}")

@mcp.tool()
async def interrogate_validator(validator_id: str, report_file_path: Optional[str] = None) -> Dict[str, Any]:
    """
            ...
            if report_file_path.lower().endswith(".csv"):
                ...
                if hasattr(tabular_report_obj, 'save'):
                    ...
                    try:
                        ...
                        gt_table = GT(tabular_report_obj) # Assumes tabular_report_obj is a DataFrame
...
def prompt_create_validator(
    ...
    """
    thresholds_msg_example = thresholds_dict_example if thresholds_dict_example else {"warning": 0.05, "error": 10} # Default example

    return (
        base.AssistantMessage("Once your data is loaded (using its `df_id`), I can create a 'Validator' object to define data quality checks."),
        base.UserMessage(
...
def prompt_get_validation_step_output(
    validator_id: str,
    step_index: int = 1,
    output_path: str = "step_1_extract.csv"
) -> tuple:
    """
...
@mcp.prompt()
def prompt_interrogate_validator(validator_id: str, report_file_path: Optional[str] = "validation_summary.csv") -> tuple:
    """
    Prompt guiding the LLM to run validations and optionally save the report.
    """
    return (
        base.AssistantMessage("After all desired validation steps have been added to a validator, I can run the interrogation process. This will execute all checks."),
        base.UserMessage(
