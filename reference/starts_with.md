...
## Relevant Validation Methods Where [starts_with()](starts_with.md#pointblank.starts_with) Can Be Used
...
Suppose we have a table with columns `name`, `paid_2021`, `paid_2022`, and `person_id` and we'd like to validate that the values in columns that start with `"paid"` are greater than `10`. We can use the [starts_with()](starts_with.md#pointblank.starts_with) column selector function to specify the columns that start with `"paid"` as the columns to validate.
