...
Note: local image paths auto-coerced through `attachments=` use `resize="low"` (chatlas's default downscale to 512x512) to keep token costs predictable. For higher fidelity, pre-build the content yourself with `chatlas.content_image_file(path, resize="high")` (or `"none"`) and pass that object inside `attachments=`.
