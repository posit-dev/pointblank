...
The [col_vals_lt()](Validate.col_vals_lt.md#pointblank.Validate.col_vals_lt) validation method checks whether column values in a table are *less than* a specified `value=` (the exact comparison used in this function is `col_val < value`). The `value=` can be specified as a single, literal value or as a column name given in <a href="../reference/col.html#pointblank.col" class="gdls-link"><code>col()</code></a>. This validation will operate over the number of test units that is equal to the number of rows in the table (determined after any `pre=` mutation has been applied).
